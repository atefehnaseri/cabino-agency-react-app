...
function SalesChart({ bookings, numDays }) {
  ...
  const areaChartData = allDates.map((date) => ({
    label: format(date, "MMM dd"),
    totalSales: getBookingsSalesData(date, "totalPrice"),
    extrasSales: getBookingsSalesData(date, "extrasPrice"),
  }));
  ...
  function getBookingsSalesData(date, key) {
    return bookings.reduce(
      (acc, curr) =>
        isSameDay(date, new Date(curr.created_at)) ? acc + curr[key] : acc,
      0
    );
  }
